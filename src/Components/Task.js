...
function Task({el}) {
    ...
    return (
        <div >
            ...
            <button style={{color:'GREEN',backgroundColor:'WHITE',border :0}} onClick={()=>handleConfirm()}><i class="bi bi-check-lg"></i></button>
            <button style={{color:'red',backgroundColor:'white',border :0}} onClick={()=>handleCancel()} ><i class="bi bi-x"></i></button>
            </div>
            :<div> {el.isDone?<span style={{textDecoration:'line-through'}}>{el.description}</span>:el.description}</div>}
             ...
             <input type="checkbox"  checked style={{display:'inline-block',zoom:'1'}} onChange={(e)=>handleIsDone(e.target.checked)} ></input>:
             <input type="radio"  style={{display:'inline-block',zoom:'1'}} onChange={(e)=>handleIsDone(e.target.checked)} ></input>}
             <button style={{border : 0, backgroundColor: 'white'}} onClick={handleEdit}><i class="bi bi-pencil-square"></i></button>
             <button style={{border : 0, backgroundColor: 'white'}}><i class="bi bi-trash" onClick={handleRemove}></i></button>
             </div>
             </div>
        </div>
    )
